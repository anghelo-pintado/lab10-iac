...
app.get("/api/hello", async (req, res) => {
  try {
    const r = await fetch(
      `${BACKEND_URL}/api/hello?name=${encodeURIComponent(req.query.name || "mundo")}`,
    );
...
app.get("/api/load", async (req, res) => {
  try {
    const r = await fetch(
      `${BACKEND_URL}/load?seconds=${parseInt(req.query.seconds, 10) || 30}`,
    );
...
const events = [
  ...
  () =>
    log("WARN", "recurso_lento", {
      asset: "main.js",
      load_ms: rand(900, 3000),
    }),
  () =>
    log("ERROR", "error_js_cliente", {
